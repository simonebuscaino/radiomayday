...
function PalinsestoToday() {
    ...
    useEffect(() => {
        const fetchTodaySchedule = async () => {
            setLoading({ is: true, text: "Caricamento palinsesto..." });
            try {
                const data = await scheduleService.getScheduleByDay(dayToday);
                setDayData(data);
            } catch (error) {
                console.error("Errore nel recupero del palinsesto:", error);
            } finally {
