                    ...
                    <div key={index} className="group">
                        <Card hover variant="default" className="flex flex-col gap-6 p-6 h-full border-neutral-100 group-hover:border-primary-200 transition-all duration-500">
                            <div className="w-full relative overflow-hidden rounded-2xl aspect-[4/5] shrink-0">
                                <Image
                                    src={el.image_url?.replace('/public', '')}
...
function StaffScreen() {
    ...
    useEffect(() => {
        const fetchStaff = async () => {
            setLoading({ is: true, text: "Caricamento staff..." });
            try {
                const allStaff = await staffService.getAllStaff();
                console.log("Dati caricati da Supabase:", allStaff);

                // Filtri basati sul nome della categoria caricata tramite join
                setVoci(allStaff.filter(s =>
                    s.categories?.name?.toLowerCase() === 'speaker' ||
                    s.categories?.name?.toLowerCase() === 'voci'
                ));

                setTecnici(allStaff.filter(s =>
                    s.categories?.name?.toLowerCase() === 'tecnico' ||
                    s.categories?.name?.toLowerCase() === 'tecnici' ||
                    s.categories?.name?.toLowerCase() === 'tecnico/regia'
                ));

                setDj(allStaff.filter(s =>
                    s.categories?.name?.toLowerCase() === 'dj'
                ));

                console.log(voci, tecnici, dj);

            } catch (error) {
                console.error("Errore nel caricamento dello staff:", error);
            } finally {
                setLoading({ is: false, text: "" });
            }
        };
