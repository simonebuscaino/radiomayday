...
function PlayerRadio() {
  ...
  const shareLive = () => {
    if (navigator.share) {
      navigator.share({
        title: 'Tropp Fun Radio',
        text: 'Sto ascoltando la prima radio interscolastica d\'Italia!',
  ...
  return (
    <div className={`fixed left-0 right-0 z-50 px-4 md:px-8 pointer-events-none transition-all duration-500 ${isOpen ? "bottom-6" : "bottom-0"}`}>
      <div className={`max-w-5xl mx-auto pointer-events-auto relative transition-all duration-500 ease-in-out transform ${
        isOpen ? "translate-y-0" : "translate-y-full scale-[0.98] sm:scale-100"
      }`}>
        
        {/* Toggle Bar - Enhanced with Micro Controls */}
        <div className="absolute bottom-full left-0 right-0 flex justify-center mb-[-1px]">
          <div className={`
            flex items-center bg-neutral-900/95 backdrop-blur-xl border border-white/10 border-b-0 rounded-t-2xl shadow-2xl transition-all duration-500
            ${!isOpen ? "px-2 py-1" : "px-4 pt-1"}
            ${!isOpen && isPlaying ? "glow-pulse border-primary-500/30" : ""}
          `}>
            ...
            >
              <div className="flex flex-col items-start">
                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-white group-hover:text-primary-400 transition-colors">
                  {isOpen ? "Chiudi Player" : (isPlaying ? "In Onda: Tropp Fun Radio" : "Ascolta la Diretta")}
                </span>
                {!isOpen && isPlaying && (
        ...
        <div className={`transition-all duration-700 ease-in-out ${
          isOpen ? "opacity-100 translate-y-0" : "opacity-0 pointer-events-none translate-y-10"
        }`}>
            ...
            <div className="flex flex-col md:flex-row items-center gap-6 relative z-10">
              ...
              <div className="flex-1 min-w-0 text-center md:text-left">
                ...
                <div className="flex items-center justify-center md:justify-start gap-3 text-neutral-400 text-sm">
                  <span className="cc_streaminfo inline-block truncate" data-type="song" data-username="eduardo">
                    In attesa di dati dalla regia...
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center justify-center gap-6 md:gap-8">
                ...
                <button 
                  ...
                  className={`w-16 h-16 md:w-20 md:h-20 flex items-center justify-center rounded-full transition-all duration-500 transform hover:scale-105 active:scale-95 ${
                    ...
                    : "bg-primary-500 text-white shadow-[0_0_30px_rgba(255,0,135,0.4)] hover:shadow-[0_0_40px_rgba(255,0,135,0.6)]"
                  }`}
                >
                  {isPlaying ? <HiPause size={36} /> : <HiPlay size={36} className="ml-1" />}
                </button>
        ...
        <audio src="https://sr11.inmystream.it/proxy/eduardo?mp=/stream" ref={refPlayer} crossOrigin="anonymous"></audio>
      </div>
    </div>
  );
}
